      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex gap-8">
        ...
        <div className="flex-1 space-y-6">
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <FormField control={form.control} name="type" render={({ field }) => (<FormItem>
                            ...
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                {roomTypes.map((type) => (<SelectItem key={type.value} value={type.value}>
                                    {type.label}
                                  </SelectItem>))}
                              </SelectContent>
                            </Select>
                      ...
                      <FormField control={form.control} name="isAvailable" render={({ field }) => (<FormItem className="flex items-end gap-4">
                            <div>
                    ...
                    <FormField control={form.control} name="description" render={({ field }) => (<FormItem>
                          ...
                          <FormControl>
                            <Textarea placeholder="Room details and amenities..." {...field}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
          ...
          <div className="grid gap-4">
            {rooms.map((room) => (<Card key={room.id} data-testid={`card-room-${room.id}`}>
                <CardContent className="pt-6">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button size="icon" variant="outline" onClick={() => handleEdit(room)} data-testid={`button-edit-room-${room.id}`}>
                        <Edit2 className="h-4 w-4"/>
                      </Button>
                      <Button size="icon" variant="destructive" onClick={() => deleteRoom.mutate(room.id)} disabled={deleteRoom.isPending} data-testid={`button-delete-room-${room.id}`}>
...
//# sourceMappingURL=rooms-admin.jsx.map
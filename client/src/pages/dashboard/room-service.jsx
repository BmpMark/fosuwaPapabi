...
export default function RoomService() {
    ...
    const myActiveReservations = reservations.filter(r => r.userId === user.id && (r.status === 'checked_in' || r.status === 'confirmed'));
    const addToCart = (id) => {
        setCart(prev => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex gap-8">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {isLoadingMenu ? (<div>Loading menu...</div>) : (<div className="grid grid-cols-1 gap-4">
                {menu.map(item => (<Card key={item.id} className="flex flex-row items-center p-4 gap-4">
                    ...
                    {cart.find(c => c.id === item.id) ? (<div className="flex items-center gap-3">
                      ...
                      </div>) : (<Button onClick={() => addToCart(item.id)}>Add</Button>)}
                  </Card>))}
              </div>)}
          </div>
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-24">
              <CardContent className="p-6 space-y-6">
                ...
                {cart.length === 0 ? (<p className="text-muted-foreground text-sm">Your cart is empty.</p>) : (<div className="space-y-4">
                    {cart.map(item => {
                const menuItem = menu.find(m => m.id === item.id);
                if (!menuItem)
                    return null;
                return (<div key={item.id} className="flex justify-between text-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={selectedRoomId} onValueChange={setSelectedRoomId}>
                        ...
                        <SelectContent>
                          {myActiveReservations.length > 0 ? (myActiveReservations.map(r => (<SelectItem key={r.id} value={r.roomId.toString()}>
                                Room {r.roomId} (Check-in: {new Date(r.checkIn).toLocaleDateString()})
                              </SelectItem>))) : (<div className="p-2 text-sm text-muted-foreground">No active bookings found</div>)}
                        </SelectContent>
                      </Select>
                    </div>
...
//# sourceMappingURL=room-service.jsx.map
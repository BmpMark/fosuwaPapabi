...
export default function RestaurantPage() {
    ...
    return (<Layout>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <Tabs defaultValue={categories[0]} className="w-full">
          <div className="flex justify-center mb-8 md:mb-12 overflow-x-auto pb-2 scrollbar-hide">
            <TabsList className="bg-muted h-auto p-1 rounded-full inline-flex">
              {categories.map((cat) => (<TabsTrigger key={cat} value={cat} className="rounded-full px-4 md:px-8 py-2 md:py-3 text-sm md:text-base capitalize data-[state=active]:bg-white data-[state=active]:shadow-sm whitespace-nowrap">
                  {cat}
                </TabsTrigger>))}
            </TabsList>
          </div>

          {categories.map((category) => (<TabsContent key={category} value={category} className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {menu
                .filter((item) => item.category === category)
                .map((item) => (<div key={item.id} className="flex gap-4 items-start group p-4 rounded-xl hover:bg-muted/30 transition-colors">
                    ...
                    <div className="space-y-1 flex-1">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        {user ? (cart.find((c) => c.id === item.id) ? (<div className="flex items-center gap-2">
                            ...
                            </div>) : (<Button onClick={() => addToCart(item.id)}>
                              Order
        ...
        {user && cart.length > 0 && (<div className="fixed bottom-6 right-6 z-40">
            <Dialog>
              ...
              <DialogContent className="max-w-md">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                    Review Your Order
                    {!isOnline && (<Badge variant="secondary" className="bg-orange-100 text-orange-800 gap-1">
                ...
                return (<div key={item.id} className="flex justify-between items-center text-sm">
                  ...
                  <Button className="w-full" onClick={submitOrder} disabled={isSubmitting || createOrder.isPending}>
                    {isSubmitting
                ? "Saving Order..."
                : isOnline
                  ...
                  {!isOnline && (<div className="text-xs text-muted-foreground text-center mt-2">
...
//# sourceMappingURL=restaurant.jsx.map
    ...
    return (<Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {rooms.map((room) => {
            const isManagerRoom = room.number === "2";
            return (<RoomCard key={room.id} room={room} user={user} isManagerRoom={isManagerRoom}/>);
        })}
        </div>
      </div>
    </Layout>);
}
function RoomCard({ room, user, isManagerRoom }) {
    ...
    const getRoomImage = (type) => {
        if (type.toLowerCase().includes("standard"))
            return standardRoomImgPath;
        if (type.toLowerCase().includes("executive"))
            return executiveRoomImgPath;
    ...
    return (<Card className="overflow-hidden group hover:shadow-xl transition-all duration-300 border-border/50 bg-card flex flex-col h-full">
      <div className="relative h-64 overflow-hidden">
        <img src={getRoomImage(room.type)} alt={room.type} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"/>
      ...
      <CardFooter className="pt-4 border-t border-border/50 gap-2">
        <Dialog open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          ...
          <DialogContent className="max-w-4xl">
            ...
            <div className="p-4 bg-white rounded-lg">
              <FullCalendar plugins={[dayGridPlugin]} initialView="dayGridMonth" events={roomEvents} headerToolbar={{
        ...
        {resolvedIsManagerRoom ? (<Button variant="outline" className="w-full cursor-not-allowed opacity-70" disabled>
            Management Only
            ...
            <DialogContent>
              ...
              {user ? (<Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                    <FormField control={form.control} name="checkIn" render={({ field }) => (<FormItem>
                    ...
                    <FormField control={form.control} name="checkOut" render={({ field }) => (<FormItem>
                ...
                </Form>) : (<div className="flex flex-col items-center justify-center py-6 space-y-4">
...
//# sourceMappingURL=rooms.jsx.map
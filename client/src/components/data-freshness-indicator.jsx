...
export function DataFreshnessIndicator({ timestamp, isOffline, className }) {
    const { isOnline } = useOnline();
    if (!timestamp && !isOffline)
        return null;
    // If explicitly marked as offline data
    if (isOffline) {
        return (<Badge variant="secondary" className="bg-orange-100 text-orange-800 gap-1">
    ...
    if (isFresh && isOnline) {
        return (<Badge variant="secondary" className="bg-green-100 text-green-800 gap-1">
...
//# sourceMappingURL=data-freshness-indicator.jsx.map
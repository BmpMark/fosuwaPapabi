...
export function PendingOrdersIndicator() {
    ...
    if (pendingOrders.length === 0)
        return null;
    return (<Card className="border-orange-200 bg-orange-50/50">
      ...
      <CardContent className="space-y-3">
        ...
        {pendingOrders.slice(0, 3).map((order) => (<div key={order.id} className="flex items-center justify-between p-2 bg-white rounded border">
            <div>
        ...
        {pendingOrders.length > 3 && (<div className="text-xs text-muted-foreground text-center">
            +{pendingOrders.length - 3} more orders
          </div>)}

        <Button onClick={handleSync} disabled={!isOnline || isSyncing} size="sm" className="w-full" variant="outline">
          {isSyncing ? (<>
...
//# sourceMappingURL=pending-orders-indicator.jsx.map
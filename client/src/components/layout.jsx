...
import { Menu, Home, Bed, Coffee, User, LogOut, LayoutDashboard, Moon, Sun, } from "lucide-react";
...
export function Navbar() {
    ...
    return (<nav className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b border-border/50">
        ...
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map((link) => (<Link key={link.href} href={link.href}>
                <div className={cn("cursor-pointer text-sm font-medium transition-colors hover:text-primary", location === link.href
                ? "text-primary"
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-[300px]">
                ...
                <div className="flex flex-col gap-4 mt-8">
                  {links.map((link) => (<Link key={link.href} href={link.href}>
                      <div className={cn("flex items-center gap-3 px-4 py-3 text-lg font-medium rounded-lg transition-colors cursor-pointer", location === link.href ? "bg-primary text-primary-foreground" : "hover:bg-muted")} onClick={() => setIsOpen(false)}>
                  ...
                  <div className="border-t border-border my-2 pt-4">
                    {user ? (<>
                        <Link href="/dashboard">
                          <div className="flex items-center gap-3 px-4 py-3 text-lg font-medium text-primary hover:bg-muted rounded-lg cursor-pointer" onClick={() => setIsOpen(false)}>
                            <LayoutDashboard className="h-5 w-5"/>
                            Dashboard
                          </div>
                        </Link>
                        <div className="flex items-center gap-3 px-4 py-3 text-lg font-medium text-destructive hover:bg-destructive/10 rounded-lg cursor-pointer" onClick={() => {
                logout.mutate();
                setIsOpen(false);
            }}>
...
export function Layout({ children }) {
    return (<div className="min-h-screen flex flex-col bg-muted/20">
